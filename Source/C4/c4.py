...
# TASK C.4: DYNAMIC DEEP LEARNING MODEL

def build_dynamic_model(layer_name='LSTM', num_layers=3, layer_size=50, dropout_rate=0.2, input_shape=(30, 1)):
    """
    Function to build a dynamic deep learning model based on specified parameters.
    Parmeters:
        - layer_name (str): Type of layer to use ('LSTM' or 'Dense').
        - num_layer (int): Number of layers to add.
        - layer_size (int): Number of units in each layer.
        - dropout_rate (float): Dropout rate for regularization.
        - input_shape (tuple): Shape of the input data.
    Returns: 
        - model (Sequential): Complied Keras model.
    """
    print(f"\n[Task C.4] Initializing {layer_name} model with {num_layers} layers...")
    ...
    if layer_name not in layer_dict:
        raise ValueError(f"Layer type '{layer_name}' is not supported. Choose from: {list(layer_dict.keys())}")

    SelectedLayer = layer_dict[layer_name]
    ...
    for i in range(num_layers):
        ...
        if i == 0:
            model.add(SelectedLayer(units=layer_sizes[i],
...
if os.path.exists(MODEL_PATH):
    ...
    model = build_dynamic_model(layer_name='LSTM',
                                num_layer=3,
                                layer_size=50,
                                dropout_rate=0.2,
